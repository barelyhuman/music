...
export default function PlayerProvider({ children }) {
  ...
  useEffect(() => {
    if (playing && currentTrackIndex >= 0) {
      play()
    }
  ...
  const getCurrentTrack = () => {
    ...
    return (queue && queue.length && queue[index]) || false
  }
  ...
  const play = () => {
    ...
    audioRef.current.onended = () => {
      progressDone()
      const trackSkipped = Math.floor((Date.now() - startedAt) / 1000)
      if (trackSkipped < 1000) {
        errorToast("Sorry, we couldn't play the requested track, skipping...")
